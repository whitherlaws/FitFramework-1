import logo from "../assets/fitframework-logo.svg";
import { Link, useMatch, useResolvedPath } from "react-router-dom";
import "../Header.scss";

export default function Header({ headerNav }) {
  return (
    <>
      <header>
        <div>
          <div>
            <HeaderLink to="/">
              <img src={logo} className="logo" alt="Fit Framework logo" />
            </HeaderLink>
          </div>
          <h1 className="profile-heading">{headerNav}</h1>
          <div className="profile">
            <HeaderLink to="/profile">
              <div className="profile-flex">
                <svg
                  className="profile-icon"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M12.5 12.5C14.1576 12.5 15.7473 11.8415 16.9194 10.6694C18.0915 9.49731 18.75 7.9076 18.75 6.25C18.75 4.5924 18.0915 3.00269 16.9194 1.83058C15.7473 0.65848 14.1576 0 12.5 0C10.8424 0 9.25268 0.65848 8.08058 1.83058C6.90848 3.00269 6.25 4.5924 6.25 6.25C6.25 7.9076 6.90848 9.49731 8.08058 10.6694C9.25268 11.8415 10.8424 12.5 12.5 12.5ZM16.6667 6.25C16.6667 7.35507 16.2277 8.41488 15.4463 9.19628C14.6649 9.97768 13.6051 10.4167 12.5 10.4167C11.3949 10.4167 10.3351 9.97768 9.55372 9.19628C8.77232 8.41488 8.33333 7.35507 8.33333 6.25C8.33333 5.14493 8.77232 4.08512 9.55372 3.30372C10.3351 2.52232 11.3949 2.08333 12.5 2.08333C13.6051 2.08333 14.6649 2.52232 15.4463 3.30372C16.2277 4.08512 16.6667 5.14493 16.6667 6.25ZM25 22.9167C25 25 22.9167 25 22.9167 25H2.08333C2.08333 25 0 25 0 22.9167C0 20.8333 2.08333 14.5833 12.5 14.5833C22.9167 14.5833 25 20.8333 25 22.9167ZM22.9167 22.9083C22.9146 22.3958 22.5958 20.8542 21.1833 19.4417C19.825 18.0833 17.2687 16.6667 12.5 16.6667C7.72917 16.6667 5.175 18.0833 3.81667 19.4417C2.40417 20.8542 2.0875 22.3958 2.08333 22.9083H22.9167Z"
                    fill="url(#paint0_linear_3471_223)"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_3471_223"
                      x1="12.5"
                      y1="0"
                      x2="12.5"
                      y2="25"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop stop-color="#FF875C" />
                      <stop offset="0.47" stop-color="#BF9ACA" />
                      <stop offset="1" stop-color="#96C5B0" />
                    </linearGradient>
                  </defs>
                </svg>
                <p>Profile</p>
              </div>
            </HeaderLink>
          </div>
        </div>
      </header>
    </>
  );
}

function HeaderLink({ to, children, ...props }) {
  const resolvedPath = useResolvedPath(to);
  const isActive = useMatch({ path: resolvedPath.pathname });

  return (
    <Link to={to} {...props} className={isActive ? "active" : ""}>
      {children}
    </Link>
  );
}
